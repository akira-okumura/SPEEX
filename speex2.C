...
class SinglePEAnalyzer {
 ...
 public:
  ...
  enum Esuminus0make {ignoreall,set_together_bin,ishida0125};
  ...
  double GetfLambda() const {return fLambda;}   //Takahashi
  double GetfLambdaErr() const {return fLambda_err_fromN ;}   //Takahashi
  double GetLambda_C() const {return cLambda;}  //Takahashi
  double GetLambda_CErr() const {return cLambda_err;}  //Takahashi
  double GetAlpha() const {return alphaH1;}  //Takahashi
  ...
  void PrintAllLambdas() {
    ...
    std::cout <<std::endl <<  "OnePE Mean:";
    for (int i=0; i<=5;++i) {std::cout << GetOnePEDist(i)->GetMean() << " " ;}
    std::cout <<std::endl;
    std::cout << "Noise StdDev" << fNoiseH1->GetStdDev() << std::endl ;
  }
  ...
  void Suminus0_MakeNegativeClean(std::shared_ptr<TH1D> Suminus0, Esuminus0make mode = ignoreall);
  void MakeNegativeCleanDefault(std::shared_ptr<TH1D> Suminus0 , bool ignore); 
  void MakeNegativeCleanIshida(std::shared_ptr<TH1D> Suminus0 ); 
  ...
  void SetParameter_byAlpha() {
    N0_MH.push_back(1.00 * Noff0 * alpha_MH.back() );
...
void SinglePEAnalyzer::MakeMCSpectra(std::shared_ptr<TH1D> noise, std::shared_ptr<TH1D> singlePE, double lambda, int nevents, bool ignorePedestalConvolution) {
  std::cout << "*MakeMCSpectra*" <<std::endl; 
  std::cout << "Noise Mean : " << noise->GetMean() <<std::endl; 
  std::cout << "Single PE Mean : " << singlePE->GetMean() <<std::endl; 
  ...
  for(int i = 0; i < nevents; ++i){
    ...
    double charge = 0;
    if (i <100) std::cout << npe << " " ;

    for(int j = 0; j < npe; ++j){
      charge += singlePE->GetRandom();
    }
    if (ignorePedestalConvolution) {
      //Trueの時、1.p.e.以上の分布はNoiseが足されなくなる
      if (npe == 0) {
  ...
  std::cout << "Signal Noise Mean : " << fSignalH1->GetMean() << "\t" << noise->GetMean() <<std::endl;
}
...
void SinglePEAnalyzer::MakefSuminus0() {
  ...
  Suminus0_MakeNegativeClean(fSuminus0,ignoreall);
  fSuminus0->SetFillColor(1);
}

void SinglePEAnalyzer::Suminus0_MakeNegativeClean(std::shared_ptr<TH1D> Suminus0 , Esuminus0make mode) {
  switch(mode) {
  case ignoreall :
    MakeNegativeCleanDefault(Suminus0, false);
    break;
  case set_together_bin :
    MakeNegativeCleanDefault(Suminus0, true);
    break;
  case ishida0125 :
    MakeNegativeCleanIshida(Suminus0);
    break;
  default:
    MakeNegativeCleanDefault(Suminus0, true);
    break;
...
void SinglePEAnalyzer::MakeNegativeCleanDefault(std::shared_ptr<TH1D> Suminus0 , bool ignore) {
  //bool ignore = false が　default
  ...
  */
  ...
  std::cout << "Suminus0->Integral() = " << Suminus0->Integral() << std::endl;
...
void SinglePEAnalyzer::MakeNegativeCleanIshida(std::shared_ptr<TH1D> Suminus0 ) {
  //bool ignore = false が　default
  // 0回りを整理する。fNoiseH1->GetStdDev()よりシグマの範囲を同じびんに放り込む
  // 1.5σの範囲にするかどうかは議論の余地がある
  double fNoiseH1_sigma_upperedge = fNoiseH1->GetMean() + fNoiseH1->GetStdDev();
  double fNoiseH1_sigma_upperedge_bin = fNoiseH1->FindBin(fNoiseH1_sigma_upperedge);
  std::cout << "MakeNegativeClean" << std::endl;
  std::cout << "fNoiseH1 StdDev = " << fNoiseH1_sigma_upperedge << " (" << fNoiseH1_sigma_upperedge_bin << " bin) 0pC bin is " << fNoiseH1->FindBin(0) <<"bin"<<std::endl;
  ...
  for (int i = 1; i <= fNoiseH1->GetNbinsX(); ++i) {
    if (fNoiseH1->GetBinCenter(i) <=
        0 * fNoiseH1_sigma_upperedge) {  //ここの倍率は議論の余地がある
      together_bin_content += Suminus0->GetBinContent(i);
      together_bin_denominator += Suminus0->GetBinContent(i);
      together_bin_numerator +=
          Suminus0->GetBinContent(i) * fNoiseH1->GetBinCenter(i);
  ...
  */
  ...
  std::cout << "Suminus0->Integral() = " << Suminus0->Integral() << std::endl;
  std::cout << "negative contents cleaner : result "<< std::endl;
  double total_negative_entries = 0 ;
  for (int i = 1; i <= fNoiseH1->GetNbinsX() ; ++i) {
    double icontent = Suminus0->GetBinContent(i);
    if (icontent < 0) {
      if (i <= fNoiseH1_sigma_upperedge_bin) {
        std::cout << i << "(" << icontent << ")  " ;
      } else {
        total_negative_entries += icontent;
      }
      Suminus0->SetBinContent(i, 0);
    } 
  }
  std::cout << "upper than sigma(bin" << fNoiseH1_sigma_upperedge_bin << ") total is " << total_negative_entries << std::endl;
  suminus0_negativecontents_upperthansigma.push_back(total_negative_entries);
  
  //01/25 かなり真の値に近くなってきたがどうなのこれ
  std::cout << "Suminus0->Integral() = " << Suminus0->Integral() << std::endl;

  std::cout << "MakeNegativeCleaneIshida 1PE mean is " << GetOnePEDist()->GetMean() << std::endl;
  int pre1pe_mean_bin = GetOnePEDist()->FindBin(GetOnePEDist()->GetMean());
  double f0entry_UnderOnePEMean = Suminus0->Integral(1,pre1pe_mean_bin);
  double scale =  ( 1 - (total_negative_entries / f0entry_UnderOnePEMean ));
  std::cout << "pre1PEMean = " << GetOnePEDist()->GetMean() << "(bin" << pre1pe_mean_bin << "), Scale = " << scale << std::endl;
  for (int i = 1; i <=pre1pe_mean_bin; ++i) {
    double pre_entry = Suminus0->Integral(i,i);
    if (pre_entry > 0) {
      std::cout << i << "bin entry: " << pre_entry << " to " << scale * pre_entry << std::endl;
      Suminus0->SetBinContent(i,scale*pre_entry);
    }
  }
...
void SinglePEAnalyzer::Iterate_Ishida2021() {
  ...
  if (n != 0) {
    ...
    Suminus0_MakeNegativeClean(h0,ishida0125);
  } else {
    h0->Add(fSuminus0.get());
  }
  //ここのfN1を変える必要がある
  ...
  //1/21 memo: 初回のみfを使用し、以降はcLmabdaを使用する手法を用いていたが、それだと収束しない気がする
...
void SinglePEAnalyzer::MakeNPEdistFast(bool Blur_with_Noise) {
  ...
  if (Blur_with_Noise == true) {
    for (int npe = 1; npe<=kMaxPE; ++npe) {
      std::vector<double> ibin_Entries = {0};
      for (int ibin = 1; ibin <= nbins; ++ibin) {
        int i = ibin - bin0;
        double n2i = 0;
        for (int jbin = 1; jbin <= nbins; ++jbin) {
          ...
          n2i +=  n1j * n1i_j / Noff0; //fNoiseH1のEntries
        }
        ibin_Entries.push_back(n2i); //従来通り h[npe]->SetBinContent(ibin, n2i);だと違う答えになる
      }
      for (int ibin = 1; ibin<= nbins; ++ibin) h[npe]->SetBinContent(ibin,ibin_Entries[ibin]);
    }
  }
...
void SinglePEAnalyzer::OnePEiterate(double N) {
  ...
  for (int i = h0->FindBin(0); i <= h0->GetNbinsX(); ++i) {
    ...
    if (sum <= N) {
      ...
      std::cout << "OnePEDist filled " << int(N) << "/" << int(N) << " complete!" << std::endl;
      break;
    }
  }
  if (int(h1->Integral()) != int(N) ) { std::cout << "OnePEDist filled " << int(h1->Integral()) << "/" << int(N) << std::endl; }
...
void Savedata_Lamb_and_Charge(std::shared_ptr<SinglePEAnalyzer> ana,std::shared_ptr<SinglePEAnalyzer> ana2) {
  ...
  out << ana->GetfLambda() << "\t" << ana->GetfLambdaErr() << "\t" << ana->GetLambda_C() << "\t" << ana->GetLambda_CErr() 
...
void SaveData_takahashi(std::shared_ptr<SinglePEAnalyzer> ana,std::shared_ptr<SinglePEAnalyzer> ana2,const std::string& file_name) {
  ...
  << "\t" << ana->GetfLambda() << "\t" << ana->GetfLambdaErr() << "\t" << ana->GetLambda_C() << "\t" << ana->GetLambda_CErr() 
  << "\t" << ana->GetOnePEDist()->Integral(1,-1) << "\t" << ana->GetOnePEDist()->GetMean() << "\t" << ana->GetOnePEDist()->GetMeanError()
  << "\t" << ana->GetOnePEDist()->GetStdDev() << "\t" << ana->GetOnePEDist()->GetStdDevError()
  << "\t" << ana2->GetOnePEDist()->Integral(1,-1) << "\t" << ana2->GetOnePEDist()->GetMean() << "\t" << ana2->GetOnePEDist()->GetMeanError()
  << "\t" << ana2->GetOnePEDist()->GetStdDev() << "\t" << ana2->GetOnePEDist()->GetStdDevError() << std::endl;
...
void SaveData_MH(std::shared_ptr<SinglePEAnalyzer> ana,std::shared_ptr<SinglePEAnalyzer> ana2,const std::string& file_name) {
  //NoiseHV,HV(/100),fLamb,fLambErr,cLamb,cLambErr,[ana]Ent,Mean,MeanErr,StdD,StdDErr,[ana]Ent...(16 properties)
  std::ofstream out;//Noise2450/turn14_CH1
  out.open("data.txt", std::ios::app);
  if (file_name[0] == 'N') {
  ...
  out << std::fixed << std::setprecision(4) << "\t" << file_name[file_name.size() -11] << file_name[file_name.size() - 10] 
  << "\t" << ana->GetLambda(5) << "\t" << ana->GetLambdaErr(5) << "\t" << ana->GetcLambda(5) << "\t" << ana->GetcLambdaErr(5) 
  << "\t" << ana->GetOnePEDist()->Integral(1,-1) << "\t" << ana->GetOnePEDist()->GetMean() << "\t" << ana->GetOnePEDist()->GetMeanError()
  << "\t" << ana->GetOnePEDist()->GetStdDev() << "\t" << ana->GetOnePEDist()->GetStdDevError()
  << "\t" << ana2->GetOnePEDist()->Integral(1,-1) << "\t" << ana2->GetOnePEDist()->GetMean() << "\t" << ana2->GetOnePEDist()->GetMeanError()
  << "\t" << ana2->GetOnePEDist()->GetStdDev() << "\t" << ana2->GetOnePEDist()->GetStdDevError() << std::endl;
...
void SaveData_Simulation_forNoise(std::shared_ptr<SinglePEAnalyzer> ana, const std::string& file_name, bool urIshida = true ) {
  //Noise	NoiseE alpha Mean	MeanE	fLamb	fLambE	cLamb	cLambE	
  std::ofstream out;//Sim/normal_gain15_NoiseXX.root
  out.open("data.txt", std::ios::app);
  for (int i=23; i<=(file_name.size() - 6) ; ++i) {out <<file_name[i];}
  
  if (urIshida) { //ishida is true
    out << std::fixed << std::setprecision(4) 
    << "\t" << int(ana->GetNoiseH1()->Integral()) << "\t" << ana->GetAlpha(5) << "\t"  << ana->GetOnePEDist()->GetMean() << "\t" << ana->GetOnePEDist()->GetMeanError()
    << "\t" << ana->GetLambda(5) << "\t" << ana->GetLambdaErr(5) << "\t" << ana->GetcLambda(5) << "\t" << ana->GetcLambdaErr(5) << std::endl;
  } else {
    out << std::fixed << std::setprecision(4) 
    << "\t" << int(ana->GetNoiseH1()->Integral()) << "\t" << ana->GetAlpha() << "\t"  << ana->GetOnePEDist()->GetMean() << "\t" << ana->GetOnePEDist()->GetMeanError()
    << "\t" << ana->GetfLambda() << "\t" << ana->GetfLambdaErr() << "\t" << ana->GetLambda_C() << "\t" << ana->GetLambda_CErr() << std::endl;
  }
  out.close();
}
...
std::pair<std::shared_ptr<SinglePEAnalyzer>, std::shared_ptr<SinglePEAnalyzer>> test(int Iteration_MaxPE = 4,const std::string& file_name = READPATH) {
  ...
  for (int i=0; i < 10 ; ++i) std::cout << ana->GetOnePEDist()->GetRandom() << " ";
  std::cout << std::endl;
  auto ana2 = std::make_shared<SinglePEAnalyzer>();
  std::cout << "ana onePEDist Mean and lambda is " << ana->GetOnePEDist()->GetMean() << "\t" << ana->GetfLambda() << std::endl;
  ana2->MakeMCSpectra(ana->GetNoiseH1(), ana->GetOnePEDist(), ana->GetfLambda(), ana->GetSignalH1()->GetEntries(), true);
  ...
  std::cout << "fSignalとfNoiseのMean, 二つの差" << std::endl;
  std::cout << ana->GetSignalH1()->GetMean() << "\t" << ana->GetNoiseH1()->GetMean() << "\t" << ana->GetSignalH1()->GetMean() - ana->GetNoiseH1()->GetMean() <<std::endl ;
  std::cout << ana2->GetSignalH1()->GetMean() << "\t" << ana2->GetNoiseH1()->GetMean() << "\t" << ana2->GetSignalH1()->GetMean() - ana2->GetNoiseH1()->GetMean()  << std::endl;
  ...
  SaveData_Simulation_forNoise(ana,file_name,false); 
...
std::pair<std::shared_ptr<SinglePEAnalyzer>, std::shared_ptr<SinglePEAnalyzer>> test_iteration_ishida(int Iteration_MaxPE = 4,const std::string& file_name = READPATH) {
  auto ana = std::make_shared<SinglePEAnalyzer>();
  std::cout << "now, iterate " ;
  for (int i=5; i<=8; ++i) {std::cout <<file_name[i]; }
  std::cout << std::endl;
  ...
  auto ana2 = std::make_shared<SinglePEAnalyzer>();
  std::cout << "ana onePEDist Mean and lambda is " << ana->GetOnePEDist()->GetMean() << "\t" << ana->GetLambda(5) << std::endl;
  ...
  SaveData_Simulation_forNoise(ana,file_name);
  two_analyzer_compare(ana,ana2);
...
std::pair<std::shared_ptr<SinglePEAnalyzer>, std::shared_ptr<SinglePEAnalyzer>> fN0_vs_kN0_test (int Iteration_MaxPE = 4,const std::string& file_name = READPATH) {
  ...
  for (int i = 0; i <= Iteration_MaxPE ; ++i ) {
    std::cout << "ana2[" << i << "] alpha, fLambda_MH, cLambda = " 
    << ana2->GetAlpha(i) << "\t" << ana2->GetLambda(i) << "\t" << ana2->GetcLambda(i) << std::endl;
  }
  ...
  for (int i = 0; i <= 5; ++i) {
    SuminusCan->cd(i+1);
    ana2->GetNoiseH1()->Draw("same");
    ana2->Getf0PEDist(i)->SetLineColor(7); //lightblue
...
void two_analyzer_compare(std::shared_ptr<SinglePEAnalyzer> ana1, std::shared_ptr<SinglePEAnalyzer> ana2) {
  ...
  comp_can->cd(3);
  fSignal_compare->GetYaxis()->SetRangeUser(-100,100);
